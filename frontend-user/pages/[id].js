...
function ProductDetails({ product, review }) {
  ...
  const handleSizeChange = (item) => {
    ...
    const selectedProduct = product.size.sizes.find(
      (size) => size._id === item._id
    );
    if (selectedProduct) {
  ...
  const content = (
    <div className="w-[500px] font-roboto text-md">
      <span>
        {" "}
        Dùng chỉ hoặc giấy bản nhỏ quấn quanh cổ tay (nếu là lắc tay) và quấn
        quanh khớp tay (nếu là nhẫn), bạn lưu ý quấn trừ hao sao cho kích cỡ
        thoải mái một chút, đánh dấu vị trí cắt nhau.
      </span>
      ...
      <span>
        Chiều dài đoạn dây bao nhiêu cm tương ứng với size lắc bấy nhiêu cm.
      </span>
    ...
    <div className="container flex flex-col mt-[50px] md:mb-[200px] mb-[50px] ">
      <div className="flex justify-center">
        {product ? (
          <div
            key={product._id}
            className="md:flex items-center lg:gap-[100px] gap-[50px] "
          >
            ...
            <div className="flex flex-col justify-center gap-3 md:mr-[100px] max-w-[400px] md:mt-0 mt-[50px]">
              <h4 className="flex gap-4 text-xl font-roboto font-bold">
                Jewelry | {product.productName}
              </h4>
              ...
              {product.discount > 0 && (
                <div className="flex font-roboto text-md justify-between">
                  <p>Giảm giá :</p>{" "}
              ...
              {product.stock > 0 && product.stock ? (
                ...
                <div className="flex justify-between">
                  <div className="font-roboto text-md text-primry">
                    Chọn kích cỡ :
                  </div>

                  <Popover content={content} title="Cách đo size">
                    <i className="underline font-roboto text-md text-primry">
                      Cách đo size nhẫn - vòng tay
                    </i>
                  </Popover>
                </div>
              )}
                ...
                {product.size &&
                  product.size.sizes.map((item) => (
                    <label
                      ...
                      className={`${
                        selectedSize === item
                          ? "bg-pink text-text-1 border-primry"
                          : "border-primry"
              ...
              {stock <= 0 && product.stock === 0 && (
                <div className="flex gap-8">
                  ...
                  <Button
                    onClick={handleAddCartSize}
                    className="text-md font-roboto bg-primry text-white min-h-[45px] hover:bg-white hover:text-primry hover:border-primry"
                  >
                    Thêm vào giỏ hàng
                  </Button>
                </div>
      ...
      {review ? (
        <div className="mt-[40px]">
          ...
          {review.length === 0 ? (
            <p className="font-roboto mt-2"> Hiện chưa có đánh giá nào</p>
          ) : (
            ""
          )}
          {review.map((item) => {
            return (
              <div
                key={item._id}
                className="flex flex-col gap-4 font-roboto text-sm"
              >
