...
function Promotion({ products }) {
    ...
    return (
        <div className="grid grid-cols-5 gap-2 container">
            {products && products.filter((item) => item.discount > 0).map((item) => {
                return (
                    <div className="sm:min-w-[15.625rem] sm:min-h-[12.5rem] min-w-[100px] min-h-[100px] shadow-md rounded hover:bg-second-3 flex flex-col justify-center items-center mt-[3.125rem] mb-[3.125rem]" style={{
                        ...
                        <div className="group relative inline-flex justify-center overflow-hidden items-center" >
                            ...
                            <span className="!absolute top-0 right-0 bg-primry font-poppins text-md font-normal py-[4px] sm:px-[24px] px-[4px] text-white">
                                -{item.discount}%
                            </span>
                        </div>

                        <div className="flex flex-col gap-[6px]">
                            <p className="font-roboto text-sm font-normal flex justify-center xxl:truncate">{item.productName}</p>
